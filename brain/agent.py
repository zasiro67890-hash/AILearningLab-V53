...
def landed_on_target(
    ...
    if was_airborne and now_grounded:
        ...
        # Player should be reasonably close
        # to the platform's top.
        expected_y = (
            target["y"] - 50
        )
        ...
        def do_POST(self):
            ...
            try:
                ...
                if self.path == "/register":
                    out = brain.register(payload["worker_id"])
...
async def train_worker(worker_id, p):
    ...
    local_episodes = 0
    try:
        safe_worker_id = f"worker_{int(worker_id):03d}"
        await brain_request("/register", {"worker_id": safe_worker_id})
        browser, context, page = await create_browser(p)
        cdp = await page.context.new_cdp_session(page)
        state = await get_state(page, cdp)
        if not state:
            # Render/Canvas can finish loading a little after the document reports
            # readyState=complete. Give the game a short grace period before aborting.
            for retry in range(1, 9):
                await asyncio.sleep(0.75)
                state = await get_state(page, cdp)
                if state:
                    print(f"✅ Worker {worker_id}: game state became available after retry {retry}")
                    break
            if not state:
                print(f"❌ Worker {worker_id}: initial state unavailable after retries")
                return
        ...
        for local_step in range(1, MAX_STEPS + 1):
            ...
            decision = await brain_request("/action", {"worker_id": safe_worker_id, "state": state})
            ...
            next_state = await get_state(page, cdp)
            if not next_state:
            ...
            trajectory.append({"state": state_key(state), "action": action, "reward": reward, "events": events[-3:]})
            if len(trajectory) > 200:
                trajectory = trajectory[-200:]
            await brain_request("/experience", {
                "worker_id": safe_worker_id, "key": key, "action": action,
                "reward": reward, "next_state": next_state, "done": died,
                "events": events, "target": target_payload, "level_up": level_up,
                "episode_reward": episode_reward + reward, "state": state,
                "trajectory": trajectory[-REPLAY_WINDOW:],
                "reason": "death" if died else ("level_up" if level_up else "failure"),
            })
            await brain_request("/heartbeat", {"worker_id": safe_worker_id, "info": {
            ...
            if local_step % 20 == 0 or events or died:
                ...
                target_text = f"{target['x']:.0f}-{target['x'] + target['width']:.0f}" if target else "NONE"
                snap = await brain_request("/stats")
                print(f"\n[W{worker_id} S{local_step}] 🤖 {action}")
                print(f"   🎮 X={next_state['player']['x']:.1f} Y={next_state['player']['y']:.1f} VX={next_state['player']['velocityX']:.1f} VY={next_state['player']['velocityY']:.1f}")
                print(f"   🎯 Target={target_text} | Level={next_state['currentLevel']}")
                print(f"   🛬 Predicted landing X={predicted_landing_x(next_state['player']):.1f}")
                print(f"   🧠 Central states={snap['states']} replay={snap['replay']} ε={snap['epsilon']}")
                print(f"   🌐 Workers online={snap['workers_online']} | Total steps={snap['steps']}")
                print(f"   📊 Deaths={next_state['deaths']} Reward={reward:.2f}")
                for event in events: print(f"   ⚡ {event}")

            if died:
                trajectory = []
                local_episodes += 1
                final_episode_reward = episode_reward + reward
                if os.getenv("AI_EVAL_MODE", "0") == "1":
                    try:
                        await brain_request("/evaluation", {
                        ...
                        print(f"⚠️ Evaluation report failed: {e}")
                print(f"\n💀 WORKER {worker_id} EPISODE {local_episodes} FINISHED | Reward={final_episode_reward:.2f}")
                episode_reward = 0.0
                episode_target = None
...
async def train_local():
    ...
    try:
        async with async_playwright() as p:
            tasks=[asyncio.create_task(train_worker(i+1,p)) for i in range(NUM_ENVS)]
...
async def train_remote(workers):
    ...
    async with async_playwright() as p:
        tasks = [asyncio.create_task(train_worker(i + 1, p)) for i in range(workers)]
        try:
